...
@Component({
  ...
  template: `
    <div class="login-container">
      <div class="login-box">
        <img src="assets/adi.png" alt="Company Logo" class="logo"> <!-- Updated path -->
        <h1>Login</h1>
        <form (ngSubmit)="onLogin()" #loginForm="ngForm">
          ...
          <button mat-raised-button color="primary" type="submit" class="login-button" [disabled]="loginForm.invalid">Login</button>
        </form>
      </div>
    </div>
  `,
...
export class LoginPage {
  ...
  onLogin() {
    if (!this.username || !this.password || this.password.length < 6 || this.username.length < 5) {
